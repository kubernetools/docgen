...
mod tests {
    ...
    fn creates_sitemap_from_scratch() {
        ...
        generate(
            &[p("https://example.com/a/"), p("https://example.com/b/")],
            &path,
            &[],
        )
        .unwrap();
    ...
    fn merges_different_versions() {
        ...
        let path = dir.path().join("sitemap.xml");
        generate(
            &[p("https://example.com/docs/v1.33/pod/")],
            &path,
            &[p("https://example.com/docs/v1.33/")],
        )
        .unwrap();
        generate(
            &[p("https://example.com/docs/v1.34/pod/")],
            &path,
            &[p("https://example.com/docs/v1.34/")],
        )
        .unwrap();
    ...
    fn evicts_stale_urls_for_regenerated_version() {
        ...
        generate(
            &[
                p("https://example.com/docs/v1.33/foo/"),
                p("https://example.com/docs/v1.33/bar/"),
            ],
            &path,
            &prefix,
        )
    ...
    fn eviction_does_not_affect_other_versions() {
        ...
        generate(
            &[
                p("https://example.com/docs/v1.33/foo/"),
                p("https://example.com/docs/v1.34/foo/"),
            ],
            &path,
            &[],
        )
        ...
        let got = loc_urls(&path);
        assert!(
            got.contains(&p("https://example.com/docs/v1.34/foo/")),
            "v1.34 must survive"
        );
        assert!(
            !got.contains(&p("https://example.com/docs/v1.33/foo/")),
            "stale v1.33 entry must be gone"
        );
        assert!(
            got.contains(&p("https://example.com/docs/v1.33/bar/")),
            "new v1.33 entry must be present"
        );
    }
